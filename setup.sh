...
if [[ ! -f .env ]]; then
  ...
  warn "Review .env and set IRIS_ADMIN_PASSWORD, MISP_ADMIN_PASSWORD, CALDERA_RED_PASS, and CALDERA_BLUE_PASS before use"
else
  ok ".env already exists"
fi

# Fail fast if a required credential is blank in .env — compose would interpolate it as an empty string.
REQUIRED_CREDS=(OPENSEARCH_INITIAL_ADMIN_PASSWORD IRIS_SECRET_KEY IRIS_ADMIN_PASSWORD IRIS_DB_PASSWORD MISP_DB_PASSWORD MISP_ADMIN_PASSWORD ST2_AUTH_TOKEN)
for var in "${REQUIRED_CREDS[@]}"; do
  value="$(grep -E "^${var}=" .env | cut -d= -f2- || true)"
  if [[ -z "${value//\"/}" ]]; then
    fail "${var} is blank in .env — that service would start with an empty password. Set it and re-run."
  fi
done
