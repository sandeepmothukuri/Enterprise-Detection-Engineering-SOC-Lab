...
for path in files:
    for doc in yaml.safe_load_all(path.read_text()):
        if not isinstance(doc, dict): raise SystemExit(f'{path}: document is not a mapping')
        for key in ('title','id','status','description','author','date','tags','falsepositives','level'):
            if key not in doc: raise SystemExit(f'{path}: missing {key}')
        if not uuid_re.match(str(doc['id'])): raise SystemExit(f'{path}: invalid UUID: {doc["id"]}')
        if doc['id'] in ids: raise SystemExit(f'duplicate Sigma id: {doc["id"]}')
        ids.add(doc['id'])
        raw_date = doc['date'].isoformat() if isinstance(doc['date'], (date, datetime)) else str(doc['date'])
        if not re.match(r'^\d{4}-\d{2}-\d{2}$', raw_date): raise SystemExit(f'{path}: date must be YYYY-MM-DD')
        tags = doc.get('tags') or []
        if not any(str(tag).startswith('attack.') for tag in tags): raise SystemExit(f'{path}: missing ATT&CK tag')
        if 'correlation' in doc:
            corr = doc['correlation']
            for key in ('type','rules','timespan','condition'):
                if key not in corr: raise SystemExit(f'{path}: correlation missing {key}')
        else:
...
if not files: raise SystemExit('no ElastAlert2 rules found')
for path in files:
    data = yaml.safe_load(path.read_text())
    missing = required - set(data)
    if missing: raise SystemExit(f'{path}: missing {sorted(missing)}')
    if data['type'] not in valid_types: raise SystemExit(f'{path}: unsupported rule type {data["type"]}')
    if not str(data['index']).startswith('soc-logs-'): raise SystemExit(f'{path}: unexpected index {data["index"]}')
    alerts = data['alert'] if isinstance(data['alert'], list) else [data['alert']]
    if 'post' in alerts and not data.get('http_post_url'): raise SystemExit(f'{path}: post alert missing http_post_url')
...
from pathlib import Path
import py_compile
files = [p for p in sorted(Path('.').rglob('*.py')) if '.git' not in p.parts]
...
required = {'OPENSEARCH_INITIAL_ADMIN_PASSWORD','IRIS_SECRET_KEY','IRIS_ADMIN_PASSWORD','IRIS_ADM_API_KEY','IRIS_DB_PASSWORD','MISP_ADMIN_PASSWORD','MISP_DB_PASSWORD','MISP_MYSQL_ROOT_PASSWORD','VELOX_PASSWORD','CALDERA_RED_PASS','CALDERA_BLUE_PASS','ST2_AUTH_TOKEN','CREWAI_API_PORT','OLLAMA_MODEL'}
missing = required - set(keys)
if missing: raise SystemExit(f'.env.example missing {sorted(missing)}')
for forbidden in ('OPENSEARCH_PASSWORD=','IRIS_API_KEY='):
    if forbidden in text: raise SystemExit(f'obsolete environment variable present: {forbidden}')
